...
public class CoordInatorMenu extends FrameLayout {
    ...
    private PlatformHomeView mPlatformHomeView;
    ...
    private static final int MENU_MARGIN_RIGHT = 80;
    ...
    private class CoordinatorCallback extends ViewDragHelper.Callback {
        ...
        public boolean tryCaptureView(View child, int pointerId) {
            return mPlatformHomeView == child || mMenuView == child;
        }

        @Override
        public void onViewCaptured(View capturedChild, int activePointerId) {
            if (capturedChild == mMenuView) {
                mViewDragHelper.captureChildView(mPlatformHomeView, activePointerId);
            }
        }
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            ...
            if (mDragOrientation == LEFT_TO_RIGHT) {
                if (xvel > SPRING_BACK_VELOCITY || mPlatformHomeView.getLeft() > mSpringBackDistance) {
                    openMenu();
                } else {
                    closeMenu();
                }
            } else if (mDragOrientation == RIGHT_TO_LEFT) {
                if (xvel < -SPRING_BACK_VELOCITY || mPlatformHomeView.getLeft() < mMenuWidth - mSpringBackDistance) {
                    closeMenu();
                } else {
                    openMenu();
                }
    ...
    protected void onFinishInflate() {
        ...
        mPlatformHomeView = (PlatformHomeView) getChildAt(1);
        mPlatformHomeView.setParent(this);
    }
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (mMenuState == MENU_OPENED) {
            ...
            mPlatformHomeView.layout(mMenuWidth, 0, mMenuWidth + mScreenWidth, bottom);
            return;
        }
    ...
    protected boolean drawChild(Canvas canvas, View child, long drawingTime) {
        ...
        int clipRight = mPlatformHomeView.getLeft();
        if (child == mMenuView) {
        ...
        int shadowLeft = mPlatformHomeView.getLeft();
    ...
    public void computeScroll() {
        ...
        if (mPlatformHomeView.getLeft() == 0) {
            mMenuState = MENU_CLOSED;
        } else if (mPlatformHomeView.getLeft() == mMenuWidth) {
            mMenuState = MENU_OPENED;
        }
    }

    public void openMenu() {
        mViewDragHelper.smoothSlideViewTo(mPlatformHomeView, mMenuWidth, 0);
        ViewCompat.postInvalidateOnAnimation(CoordInatorMenu.this);
    }

    public void closeMenu() {
        mViewDragHelper.smoothSlideViewTo(mPlatformHomeView, 0, 0);
        ViewCompat.postInvalidateOnAnimation(CoordInatorMenu.this);
    }
